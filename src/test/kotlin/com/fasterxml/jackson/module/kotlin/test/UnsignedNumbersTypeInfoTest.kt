...
internal class UnsignedNumbersTypeInfoTest {
    ...
    // The unsigned types are final, so default typing other than EVERYTHING
    // does not apply to properties declared with them.
    @Test
    ...
    data class NullableHolder(val nullable: UInt?)

    // A nullable unsigned property is boxed on the JVM, so EVERYTHING applies type information to it.
    // EVERYTHING also adds a type id to the root object, so only the property is compared.
    @Test
    fun `applied to nullable declared property under EVERYTHING`() {
        val mapper = jacksonMapperBuilder()
            .activateDefaultTyping(typeValidator, DefaultTyping.EVERYTHING, As.PROPERTY)
            .build()
        val expected = NullableHolder(2u)
        val json = mapper.writeValueAsString(expected)

        assertEquals(defaultMapper.readTree("""["kotlin.UInt",2]"""), defaultMapper.readTree(json)["nullable"])
        assertEquals(expected, mapper.readValue<NullableHolder>(json))
    }
}
