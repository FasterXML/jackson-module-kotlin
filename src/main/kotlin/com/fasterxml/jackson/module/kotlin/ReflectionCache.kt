...
internal class ReflectionCache(reflectionCacheSize: Int) {
    ...
    private val kotlinGeneratedMethod = LRUMap<Member, Boolean>(reflectionCacheSize, reflectionCacheSize)
    ...
    fun isKotlinGeneratedMethod(key: Member, calc: (Member) -> Boolean): Boolean = kotlinGeneratedMethod.get(key) ?: calc(key).let { kotlinGeneratedMethod.putIfAbsent(key, it) ?: it }
}
