 ...
 *
 * @throws DatabindException if [T] is non-null but `null` was deserialized.
 ...
 * the check is performed per value, so [DatabindException] is thrown from
 ...
 *
 * @throws DatabindException if [T] is non-null but `null` was converted.
 *   Returning it as-is would silently break null safety, so it is reported as an error instead.
